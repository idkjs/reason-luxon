...
let fromHTTP =
  ...
  (string, {zone, setZone, locale, numberingSystem, outputCalendar});
...
let fromJSDate = (~zone=?, date) => (date, {zone: zone});
...
let fromMillis =
  ...
  (float, {zone, locale, numberingSystem, outputCalendar});
...
let invalid = (~explanation=?, reason) => (reason, explanation);
